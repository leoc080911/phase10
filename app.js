...
function normalizeState(){
  if(!state.petanque) state.petanque = { players: [], rounds: [], mode: 'simple', tournament: { matches: [] } };
  if(!state.petanque.mode) state.petanque.mode = 'simple';
  if(!state.petanque.tournament) state.petanque.tournament = { matches: [] };
  if(!state.petanque.tournament.matches) state.petanque.tournament.matches = [];
  if(!state.petanque.players) state.petanque.players = [];
  if(!state.petanque.rounds) state.petanque.rounds = [];

  if(!state.uno) state.uno = { players: [], rounds: [], target: 500 };
  if(!state.uno.target) state.uno.target = 500;

  if(!state.risk) state.risk = { players: [], continents: {} };
  if(!state.risk.continents) state.risk.continents = {};
  ['amnord','amsud','europe','afrique','asie','oceanie'].forEach(k=>{
    if(!(k in state.risk.continents)) state.risk.continents[k] = null;
  });
  state.risk.players.forEach(p=>{ if(typeof p.territories !== 'number') p.territories = 0; });

  if(!state.generic) state.generic = {};
  if(!state.phase10) state.phase10 = { players: [], rounds: [] };
  if(!state.skyjo) state.skyjo = { players: [], rounds: [] };
}
